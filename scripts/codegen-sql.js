...
function generateQueryFile() {
    try {
        const files = fs.readdirSync(inputDir).filter((file) => file.endsWith(".sql"));

        const queries = {};
        files.forEach((file) => {
            const key = `k${convertToCamelCase(file.slice(0, -4))}`;
            const sql = fs.readFileSync(path.join(inputDir, file), "utf8").trim();
            queries[key] = sql;
        });

        const strContent = `\n/* THIS FILE IS AUTOGENERATED, DON'T EDIT! */\n
                            export const sqlQueries = ${JSON.stringify(queries, null, 4)};\n`;
